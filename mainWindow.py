...
class mainWindow():
    ...
    def __init__(self, bar1, bar2, ball, width=620, height=480):
        ...
        self.fontscore = PG.font.Font('PressStartFont/PressStart2P-vaV7.ttf', 20)

        self.text = self.font.render('PongArena', True, (0, 255, 0), (0, 0, 255))
    ...
    def Intermediary_Selection_Screen(self):
        ...
        while running:
            ...
            # Init Fonts
            self.font = PG.font.Font('PressStartFont/PressStart2P-vaV7.ttf', 25)
            self.fontscore = PG.font.Font('PressStartFont/PressStart2P-vaV7.ttf', 20)
            ...
            # First Menu Item
            if SELECTION != 1:
                text = self.fontscore.render('Player VS Player', True, (0, 255, 0), (0, 0, 255))
            else:
                text = self.fontscore.render('Player VS Player', True, (0, 0, 255), (0, 255, 0))
            textRect = self.text.get_rect()
            textRect.center = (mainWindow.screenwidth // 3, mainWindow.screenheight // 3.5)
            self.screen.blit(text, textRect)

            # Second Menu Item
            if SELECTION != 2:
                text = self.fontscore.render('Player VS Computer', True, (0, 255, 0), (0, 0, 255))
            else:
                text = self.fontscore.render('Player VS Computer', True, (0, 0, 255), (0, 255, 0))

            textRect = self.text.get_rect()
            textRect.center = (mainWindow.screenwidth // 3, mainWindow.screenheight // 2.5)
            self.screen.blit(text, textRect)

            # Third Menu Item
            if SELECTION != 3:
                text = self.fontscore.render('Computer VS Computer', True, (0, 255, 0), (0, 0, 255))
            else:
                text = self.fontscore.render('Computer VS Computer', True, (0, 0, 255), (0, 255, 0))
            ...
            textRect = self.text.get_rect()
            textRect.center = (mainWindow.screenwidth // 3, mainWindow.screenheight // 1.45)
            self.screen.blit(text, textRect)
    ...
    def DisplayWinnerPart(self):
        ...
        while running:
            ...
            if mainWindow.ScorePlayer1 == 20:
                text = self.fontscore.render('Player1 has won the game!', True, (0, 255, 0), (0, 0, 255))
                textRect = self.text.get_rect()
                textRect.center = (mainWindow.screenwidth // 3, mainWindow.screenheight // 1.5)
                self.screen.blit(text, textRect)
            if mainWindow.ScorePlayer2 == 20:
                text = self.fontscore.render('Player2 has won the game!', True, (0, 255, 0), (0, 0, 255))
                textRect = self.text.get_rect()
                textRect.center = (mainWindow.screenwidth // 3, mainWindow.screenheight // 1.5)
                self.screen.blit(text, textRect)
            text = self.fontscore.render('Press any key to continue', True, (0, 255, 0), (0, 0, 255))
            textRect = self.text.get_rect()
            textRect.center = (mainWindow.screenwidth // 3, mainWindow.screenheight // 2)
            self.screen.blit(text, textRect)
            PG.display.flip()
